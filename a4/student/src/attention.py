...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    theta = [10000 ** (-2 * (i - 1) / dim) for i in range(dim // 2)]
    rope_cache = [[[math.cos(t * theta[i]), math.sin(t * theta[i])] for i in range(dim // 2)] for t in
                  range(max_positions)]
    rope_cache = torch.tensor(rope_cache)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    rotated_x = torch.view_as_complex(x.view(*x.shape[:-1], -1, 2))
    rotated_x = rotated_x * torch.view_as_complex(rope_cache[:x.shape[2], :])
    rotated_x = torch.view_as_real(rotated_x).contiguous().view(*x.shape[:-1], -1)
    ### END YOUR CODE ###
    return rotated_x
