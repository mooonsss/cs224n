...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document = document[:random.randint(4, self.block_size * 7 // 8)]

        mask_content_length = random.randint(1, len(document) // 2)
        prefix_length = random.randint(1, len(document) // 2)
        prefix = document[:prefix_length]
        mask_content = document[prefix_length:prefix_length + mask_content_length]
        suffix = document[prefix_length + mask_content_length:]
