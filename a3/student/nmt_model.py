...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 1. Construct embedding tensor X: (src_len, b, e)
        X = self.model_embeddings.source(source_padded)  # (src_len, b, e)
        if X.dim() == 2:  # batch_size=1, src_len missing
            X = X.unsqueeze(1)  # (src_len, 1, e)

        # 2. Apply post_embed_cnn
        X = X.permute(1, 2, 0)  # (b, e, src_len)
        X = self.post_embed_cnn(X)  # (b, e, src_len)
        X = X.permute(2, 0, 1)  # back to (src_len, b, e)

        # 3. Pack padded sequence and feed to encoder
        ...
        # 4. Compute decoder initial states
        # last_hidden/last_cell: (2, b, h) -> concat forwards/backwards -> (b, 2*h)
        ...
        init_decoder_hidden = self.h_projection(last_hidden_cat)  # (b, h_dec)
        init_decoder_cell = self.c_projection(last_cell_cat)  # (b, h_dec)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 2. 通过目标语言嵌入层得到 Y: (tgt_len, b, e)
        Y = self.model_embeddings.target(target_padded)

        # 3. 按时间步遍历 Y
        for Y_t in torch.split(Y, 1, dim=0):  # Y_t: (1, b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/generated/torch.squeeze.html
        # 1. 将 Ybar_t 输入解码器 LSTM Cell
        dec_state = self.decoder(Ybar_t, dec_state)  # dec_state: (h_t, cell_t)
        # 2. 取出新的隐状态
        dec_hidden = dec_state[0]  # (b, h)
        # 3. 计算注意力分数 e_t
        # dec_hidden: (b, h) → unsqueeze(2) → (b, h, 1)
        # enc_hiddens_proj: (b, src_len, h)
        # bmm → (b, src_len, 1) → squeeze(2) → (b, src_len)
